...
def description_keywords(content: str, min: int = 5, keywords: list[str] | None = None, **_) -> tuple[bool, str]:
    ...
    if keywords:
        ...
        domain_keywords = [
            "dotnet", "c#", "csharp", "asp.net", "aspnet", "ef core", "efcore",
...
def valid_skill_refs(content: str, plugin_root: str = "", **_) -> tuple[bool, str]:
    """Check all /dotnet: or skill name references point to existing skills."""
    if not plugin_root:
        # Try to find plugin root relative to common paths
        for candidate in [
            "plugins/dotnet",
            os.path.join(os.path.dirname(os.path.abspath(__file__)), "..", "..", "plugins", "dotnet"),
        ]:
    ...
    # Find /dotnet: references
    refs = re.findall(r'/dotnet:(\w[\w-]*)', content)
    # Find skill name references in backticks
    refs += re.findall(r'`(\w[\w-]+)`\s+skill', content)

    missing = []
    for ref in set(refs):
        # Normalize: dotnet:n1-check -> n1-check
        skill_name = ref.replace("dotnet:", "")
...
def valid_agent_refs(content: str, plugin_root: str = "", **_) -> tuple[bool, str]:
    ...
    for ref in set(refs):
        if ref in builtin_agents:
            continue
        # Strip dotnet: prefix if present
        clean_ref = ref.replace("dotnet:", "")
...
def specificity_ratio(content: str, min_ratio: float = 0.3, **_) -> tuple[bool, str]:
    ...
    concrete_patterns = [
        ...
        r'/\w+[/\w]*\.\w+',  # File paths
        r'dotnet \w+',           # dotnet CLI commands
        r'--\w+',             # CLI flags
        r'\w+\.cs',      # C# file names
