...
run_skills() {
    ...
    if [ "$filter" = "changed" ]; then
        ...
        changed_files=$(git diff --name-only HEAD -- 'plugins/dotnet/skills/' 2>/dev/null || echo "")
        local staged
        staged=$(git diff --cached --name-only -- 'plugins/dotnet/skills/' 2>/dev/null || echo "")
        if [ -n "$staged" ]; then
        ...
        if [ -f "$LAST_EVAL_FILE" ]; then
            ...
            local since_last
            since_last=$(git diff --name-only "$last_commit" HEAD -- 'plugins/dotnet/skills/' 2>/dev/null || echo "")
        ...
        while IFS= read -r file; do
            local skill_name
            skill_name=$(echo "$file" | sed -n 's|plugins/dotnet/skills/\([^/]*\)/.*|\1|p')
    ...
    if [ "$filter" = "all" ] || [ ${#skills_to_check[@]} -eq 0 ]; then
        ...
        for skill in "${skills_to_check[@]}"; do
            local path="plugins/dotnet/skills/$skill/SKILL.md"
            [ -f "$path" ] || continue
            local score
...
run_agents() {
    ...
    if [ "$filter" = "changed" ]; then
        local changed=""
        # Uncommitted + staged
        changed=$(git diff --name-only HEAD -- 'plugins/dotnet/agents/' 2>/dev/null || echo "")
        local staged
        staged=$(git diff --cached --name-only -- 'plugins/dotnet/agents/' 2>/dev/null || echo "")
        ...
        if [ -f "$LAST_EVAL_FILE" ]; then
            ...
            last_commit=$(cat "$LAST_EVAL_FILE")
            local since_last
            since_last=$(git diff --name-only "$last_commit" HEAD -- 'plugins/dotnet/agents/' 2>/dev/null || echo "")
