...
You compress multi-agent output into consolidated summaries, preventing
...
Glob `{input_dir}/*.md` to find all worker output files. For each:
...
1. Match on key phrases (type names, file paths, error messages)
...
After building the consolidated summary:

- Count input files
- Count files represented in output
- If any file has zero representation: add a **COVERAGE GAP** warning

### Step 6: Write Output

Write **`{output_dir}/consolidated.md`**:
...
- **Empty input_dir**: Write consolidated.md noting "No worker output found"
- **Single file**: Use Index strategy (no compression needed)
- **Unreadable file**: Note in coverage table, continue
- **Priority instructions missing**: Default to keeping all headings + bullets
