...
for dir in .claude/plans/*/; do
  ...
  if [ "$UNCHECKED" -gt 0 ]; then
    SLUG="$(basename "$dir")"
    echo "↻ Plan '${SLUG}' has ${UNCHECKED} remaining tasks (${CHECKED} done). Resume with: /dotnet:work .claude/plans/${SLUG}/plan.md"
    FOUND_PLAN=true
  fi
done

if [ "$FOUND_PLAN" = false ]; then
  echo ".NET AI Toolkit loaded — describe your task and I'll suggest the right workflow"
fi
